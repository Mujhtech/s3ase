...
func newBodyReader(ctx context.Context, w http.ResponseWriter, r *http.Request, maxSize int64) *bodyReader {
	return &bodyReader{
		ctx:        ctx,
...
func (r *bodyReader) Read(b []byte) (int, error) {
	...
	if err != nil {
		...
		if err == io.ErrClosedPipe || err == io.ErrUnexpectedEOF {
			err = fmt.Errorf("upload exceed eof %w", err)
		}
		...
		if errors.As(err, &maxBytesErr) {
			err = fmt.Errorf("request body size exceeds limit")
		}
...
func (r *bodyReader) closeWithError(err error) {
	...
	// if err := r.r.SetReadDeadline(time.Now()); err != nil {
