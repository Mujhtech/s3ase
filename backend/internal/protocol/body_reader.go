...
func (r *bodyReader) Read(b []byte) (int, error) {
	...
	if err != nil {
		...
		if err == io.ErrClosedPipe || err == io.ErrUnexpectedEOF {
			err = tusdS3Handler.ErrUnexpectedEOF
		}
		...
		if strings.HasSuffix(err.Error(), "read: connection reset by peer") {
			err = tusdS3Handler.ErrConnectionReset
		}

		// For timeouts, we also send a nicer response to the clients.
		if netErr, ok := err.(net.Error); ok && netErr.Timeout() {
			err = tusdS3Handler.ErrReadTimeout
		}
		...
		if errors.As(err, &maxBytesErr) {
			err = tusdS3Handler.ErrSizeExceeded
		}
